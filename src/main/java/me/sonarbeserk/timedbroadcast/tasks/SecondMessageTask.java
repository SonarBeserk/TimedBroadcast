    ...
    public void addPersistedData() {
        ...
        if(plugin.getConfig().getBoolean("settings.resume-on-restart")) {
            ...
            for(String timeString: messageTimes) {
                ...
                if(!plugin.getConfig().getBoolean("settings.messages." + split[0] + ".enabled") || plugin.getConfig().getString("settings.messages." + split[0] + ".message-text").equalsIgnoreCase("") || plugin.getConfig().getString("settings.messages." + split[0] + ".message-text").equalsIgnoreCase("") || !plugin.getConfig().getString("settings.messages." + split[0] + ".time-unit").equalsIgnoreCase("second") || plugin.getConfig().getInt("settings.messages." + split[0] + ".time-interval") == 0) {continue;}
                /* Bound Checking */

                if(!split[1].equalsIgnoreCase("second")) {continue;}
